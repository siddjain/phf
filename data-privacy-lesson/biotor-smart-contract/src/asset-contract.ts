...
export class AssetContract extends Contract {
    ...
    */
    ...
    @Transaction()
    public async update(ctx: Context, assetId: string, newFingerprint: string): Promise<void> {        
        const buffer = await ctx.stub.getState(assetId);
        if (buffer && buffer.length > 0) {
            const oldFingerprint = buffer.toString();
            const mspId = ctx.clientIdentity.getMSPID();
            // get the key to "unlock" the fingerprint stored on the blockchain
            const transientMap = ctx.stub.getTransient();
            const key = transientMap.get("key");
            const saltedMsp = Buffer.concat([key, Buffer.from(mspId)]);
            const digest = hash256(saltedMsp).toString("base64");
            if (digest === oldFingerprint) {
                await ctx.stub.putState(assetId, Buffer.from(newFingerprint));                
            } else {
